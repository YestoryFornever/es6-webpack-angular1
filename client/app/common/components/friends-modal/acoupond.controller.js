...
class AcoupondController {
	constructor(NetAcoupondService, $state, $stateParams) {
		"ngInject";
		this.name = 'acoupond';
	...
	friendgroups() {
			...
			let promise = this.NetAcoupondService.myFriends(this.friengsgroups);
			promise.then((res) => {
				console.log(res);
	...
	getLists() {
			...
			let promise = this.NetAcoupondService.groupMy(this.groupList);
			promise.then((res) => {
				console.log(res);
	...
	searchAllGroup() {
			...
			let promise = this.NetAcoupondService.searchFriendsGroups(this.searchInfo);
			promise.then((res) => {
				console.log(res);
				if (res.data) {
					if (res.data.status == '0') {
	...
	sendOffer() {
		...
		if (array_1.length > 0) {
			...
			let promise = this.NetAcoupondService.sendBondPrice(this.sendId);
			promise.then((res) => {
				console.log(res);
				if (res.data) {
					if (res.data.status == '0') {
